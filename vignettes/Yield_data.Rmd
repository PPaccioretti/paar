...
The protocol proposed by [@Vega2019], is implemented in the function `depurate` and consists of three steps: 1. Remove border observations (*edges*). 2. Remove global outliers (*outliers*). 3. Remove spatial outliers (*inliers*).
...
The `summary` function can be used to get a summary of the percentage of considered ouliers and the number of observations removed. The `summary` function returns a `data.frame` object.
